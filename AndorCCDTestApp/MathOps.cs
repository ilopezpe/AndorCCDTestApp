...
namespace AndorCCDTestApp
{
    class MathOps
    {
        ...
        public static void MatrixSum(IList<int> a, IList<int> b)
        {
            for (int i = 0; i < a.Count; i++)
                a[i] += b[i];
        }

        public static void MatrixSum(IList<double> a, IList<int> b)
        {
            for (int i = 0; i < a.Count; i++)
                a[i] += b[i];
        }

        public static void MatrixSum(IList<double> a, IList<double> b)
        {
            for (int i = 0; i < a.Count; i++)
                a[i] += b[i];
        }
        ...
        public static void ColumnSum(IList<double> accumulator, double[] matrix)
        {
            for (int i = 0; i < matrix.Length / accumulator.Count; i++)
            {
                ...
                MatrixSum(accumulator, new ArraySegment<double>(matrix, start, accumulator.Count));
            }
        }
        ...
        public static void ColumnSum(IList<int> accumulator, int[] matrix)
        {
            ...
            {
                int start = i * accumulator.Count;
                MatrixSum(accumulator, new ArraySegment<int>(matrix, start, accumulator.Count));
            }
        }
        ...
        public static void ColumnSumInPlace(int[] matrix, int width)
        {
            ...
            {
                int start = i * width;
                MatrixSum(LastRow, new ArraySegment<int>(matrix, start, width));
            }
        }
